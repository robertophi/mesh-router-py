...
class ControlsManager():
    def __init__(self, canvas, frame):
        ...
        self.bsize = 30
        self.bborder = 10
        self.button1 = Button(canvas, text='R', fg='black',
                              command=self.b1_command)
        self.button2 = Button(canvas, text='S', fg='black',
                              command=self.b2_command)
        self.button3 = Button(canvas, text='C', fg='black',
                              command=self.b3_command)
        self.button4 = Button(canvas, text='?', fg='black',
                              command=self.b4_command)

        self.button1.place(x=20, y=50, width=self.bsize, height=self.bsize)
        self.button2.place(x=20, y=50+1*(self.bsize+self.bborder),
                           width=self.bsize, height=self.bsize)
        self.button3.place(x=20, y=50+2*(self.bsize+self.bborder),
                           width=self.bsize, height=self.bsize)
        self.button4.place(x=20, y=50+3*(self.bsize+self.bborder),
                           width=self.bsize, height=self.bsize)
